clc;clear all;
global e1;
tol = 1e-6; de = 0.0001; e1 = -0.96;
%xturn = 0;
xturn = -sqrt(2*(e1+1));
[x1,ul] = ode45(@test3,[-1,xturn],[0 0.0001]);%从左往右积分
[x2,ur] = ode45(@test3,[1,xturn],[0 0.0001]);%从右往左积分
...
while (abs(de)>tol)
    ...
    [x1,ul] = ode45(@test3,[-1,xturn],[0 0.0001]);
    [x2,ur] = ode45(@test3,[1,xturn],[0 0.0001]);
    temp3 = ur(length(ur),1);%归一化第一步
    ur(:,:) = ul(length(x1),1)./temp3.*ur(:,:);%归一化第二步
